...
SELECT COMPOSITE(REPEAT('1', 100000), 1);
ECHO BOTH $IF $NEQ $STATE OK "PASSED" "***FAILED";
SET ARGV[$LIF] $+ $ARGV[$LIF] 1;
ECHO BOTH ": composite() limit STATE=" $STATE " MESSAGE=" $MESSAGE "\n";

SELECT ROWVECTOR_DIGIT_SORT('-99999999.99999999', 1, 1);
ECHO BOTH $IF $NEQ $STATE OK "PASSED" "***FAILED";
SET ARGV[$LIF] $+ $ARGV[$LIF] 1;
ECHO BOTH ":  ROWVECTOR_DIGIT_SORT arg ck STATE=" $STATE " MESSAGE=" $MESSAGE "\n";

SELECT SOAP_CALL('1', '1', '1', '1', '''1''999999999999999999999999999');
ECHO BOTH $IF $NEQ $STATE OK "PASSED" "***FAILED";
SET ARGV[$LIF] $+ $ARGV[$LIF] 1;
ECHO BOTH ":  SOAP_CALL arg ck STATE=" $STATE " MESSAGE=" $MESSAGE "\n";

SELECT SOAP_CLIENT('1', '1', '1', '-9999999.9999999e-1');
ECHO BOTH $IF $NEQ $STATE OK "PASSED" "***FAILED";
SET ARGV[$LIF] $+ $ARGV[$LIF] 1;
ECHO BOTH ":  SOAP_CALL_NEW arg ck STATE=" $STATE " MESSAGE=" $MESSAGE "\n";

SELECT XENC_GET_KEY_ALGO(NULL);
ECHO BOTH $IF $EQU $STATE OK "PASSED" "***FAILED";
SET ARGV[$LIF] $+ $ARGV[$LIF] 1;
ECHO BOTH ":  XENC_GET_KEY_ALGO arg ck STATE=" $STATE " MESSAGE=" $MESSAGE "\n";

SELECT XENC_GET_KEY_IDENTIFIER(NULL);
ECHO BOTH $IF $EQU $STATE OK "PASSED" "***FAILED";
SET ARGV[$LIF] $+ $ARGV[$LIF] 1;
ECHO BOTH ":  XENC_GET_KEY_IDENTIFIER arg ck STATE=" $STATE " MESSAGE=" $MESSAGE "\n";

SELECT XPF_EXTENSION('', 1, 1);
ECHO BOTH $IF $EQU $STATE OK "PASSED" "***FAILED";
SET ARGV[$LIF] $+ $ARGV[$LIF] 1;
ECHO BOTH ":  XPF_EXTENSION arg ck STATE=" $STATE " MESSAGE=" $MESSAGE "\n";

SELECT CFG_WRITE('dummy.cfg', '1', REPEAT('1', 262144), '1');
ECHO BOTH $IF $EQU $STATE OK "PASSED" "***FAILED";
SET ARGV[$LIF] $+ $ARGV[$LIF] 1;
ECHO BOTH ":  CFG_WRITE STATE=" $STATE " MESSAGE=" $MESSAGE "\n";

SELECT HTTP_LISTEN_HOST('1', 0.9999999999999, '1');
ECHO BOTH $IF $NEQ $STATE OK "PASSED" "***FAILED";
SET ARGV[$LIF] $+ $ARGV[$LIF] 1;
ECHO BOTH ":  HTTPS_LISTEN_HOST arg ck STATE=" $STATE " MESSAGE=" $MESSAGE "\n";

SELECT SPARQL_REXEC(NULL, NULL, NULL, 'NULL', NULL, NULL, NULL);
ECHO BOTH $IF $NEQ $STATE OK "PASSED" "***FAILED";
SET ARGV[$LIF] $+ $ARGV[$LIF] 1;
ECHO BOTH ":  SPARQL_REXEC arg ck STATE=" $STATE " MESSAGE=" $MESSAGE "\n";

SELECT XTE_NODEBLD_ACC();
ECHO BOTH $IF $NEQ $STATE OK "PASSED" "***FAILED";
SET ARGV[$LIF] $+ $ARGV[$LIF] 1;
ECHO BOTH ":  XTE_NODEBLD_ACC arg ck STATE=" $STATE " MESSAGE=" $MESSAGE "\n";

SELECT XMLATTRIBUTES(*);
ECHO BOTH $IF $NEQ $STATE OK "PASSED" "***FAILED";
SET ARGV[$LIF] $+ $ARGV[$LIF] 1;
ECHO BOTH ":  XMLATTRIBUTES arg ck STATE=" $STATE " MESSAGE=" $MESSAGE "\n";

SELECT XMLELEMENT(*);
ECHO BOTH $IF $NEQ $STATE OK "PASSED" "***FAILED";
SET ARGV[$LIF] $+ $ARGV[$LIF] 1;
ECHO BOTH ":  XMLELEMENT arg ck STATE=" $STATE " MESSAGE=" $MESSAGE "\n";

SELECT XMLFOREST(*);
ECHO BOTH $IF $NEQ $STATE OK "PASSED" "***FAILED";
SET ARGV[$LIF] $+ $ARGV[$LIF] 1;
ECHO BOTH ":  XMLFOREST arg ck STATE=" $STATE " MESSAGE=" $MESSAGE "\n";
