...
set U{caseno} case1279;
SELECT CAST('-39999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999' AS DECIMAL);
ECHO BOTH $IF $NEQ $STATE OK "PASSED" "***FAILED";
SET ARGV[$LIF] $+ $ARGV[$LIF] 1;
ECHO BOTH ": " $U{caseno} " STATE=" $STATE " MESSAGE=" $MESSAGE "\n";

-- set U{caseno} case1280;
-- SELECT REGEXP_LIKE('A', REPEAT('(?i)(?:', 100000));
-- ECHO BOTH $IF $NEQ $STATE OK "PASSED" "***FAILED";
-- SET ARGV[$LIF] $+ $ARGV[$LIF] 1;
-- ECHO BOTH ": " $U{caseno} " STATE=" $STATE " MESSAGE=" $MESSAGE "\n";

set U{caseno} case1281;
SELECT REGEXP_REPLACE('a', 'a', 'a', 9999999999999999999.9999999999999999999e-1, 0, NULL);
ECHO BOTH $IF $NEQ $STATE OK "PASSED" "***FAILED";
SET ARGV[$LIF] $+ $ARGV[$LIF] 1;
ECHO BOTH ": " $U{caseno} " STATE=" $STATE " MESSAGE=" $MESSAGE "\n";

set U{caseno} case1282;
SELECT REPEAT('     ', 6000999999999999999);
ECHO BOTH $IF $NEQ $STATE OK "PASSED" "***FAILED";
SET ARGV[$LIF] $+ $ARGV[$LIF] 1;
ECHO BOTH ": " $U{caseno} " STATE=" $STATE " MESSAGE=" $MESSAGE "\n";

set U{caseno} case1283;
SELECT REPLACE(REPEAT('a', 100000), 'a', CONCAT(REPEAT('a', 10000)));
ECHO BOTH $IF $NEQ $STATE OK "PASSED" "***FAILED";
SET ARGV[$LIF] $+ $ARGV[$LIF] 1;
ECHO BOTH ": " $U{caseno} " STATE=" $STATE " MESSAGE=" $MESSAGE "\n";

set U{caseno} case1284;
SELECT ST_TRANSFORM(ST_GEOMFROMTEXT('POINT(-90 -90)', 4326), 1000004326);
ECHO BOTH $IF $NEQ $STATE OK "PASSED" "***FAILED";
SET ARGV[$LIF] $+ $ARGV[$LIF] 1;
ECHO BOTH ": " $U{caseno} " STATE=" $STATE " MESSAGE=" $MESSAGE "\n";

set U{caseno} case1285;
