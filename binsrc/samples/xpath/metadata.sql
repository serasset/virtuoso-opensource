...
create procedure "XP"."XP"."LIST_TEST_FILES" () returns any
{
  return vector (
    ...
    vector ( 'book.dtd'		, '1.1.9. Use Case "XMP", Q1-Q6, DTD'		),
    vector ( 'book1.xml'	, '1.1.10. Use Case "XMP", Q1-Q6, data'		),
...
create procedure "XP"."XP"."LIST_MENU_ITEMS"() returns any
{
  return vector (
    ...
    vector (5, 'XPath Language', 'http://www.w3c.org/TR/xpath'),
...
create procedure "XP"."XP"."LIST_CASE_COMMENTS"() returns any
{
  return vector (
    vector ('1.1' , 'Use Case XMP'	, 'Experiences and Exemplars'		, '                                         '	,
    ...
    vector ('1.3' , 'Use Case SEQ'	, 'Queries based on Sequence'		, ''	,
...
Although sequence is not significant in most traditional database systems or object systems, it can be quite significant in structured documents. This use case presents a series of queries based on a medical report.'
    ...
    vector ('1.4' , 'Use Case R'		, 'Access to Relational Data'		, ''	,
...
Note that namespace prefixes must be resolved to their Namespace URIs before matching namespace qualified names. It is not sufficient to use the literal prefixes to denote namespaces. Furthermore, there are several possible ways to represent namespace declarations. Therefore, processing must be done on the namespace processed XML Information Set, not on the XML text representation.'
	...
	'This use case illustrates how a recursive query might be used.
...
create procedure "XP"."XP"."GET_DAV_DATA" (in name varchar) returns any
{
  declare data, resp any;
  declare h, req varchar;
  h := concat('http://localhost:', "XP"."XP"."EXTRACT_PORT_FROM_INI" (), '/DAV/', name);
  resp := null;
  req := 'GET';
...
grant execute on "XP"."XP"."GET_DAV_DATA" to public;
